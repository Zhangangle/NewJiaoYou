...
public class IOUtils {
	...
	private static final String APP_ROOTPATH = "zmv";
	private static final String SQLITE_DATABASEPATH = "android";
	private static final String SQLITE_DATABASEFILE = "zmv.db";
	private static final String SQLITE_DATABASEFILE2 = "dialog.db";
	private static final String UPDATEFILE = "zmv.apk";
	...
	public final static String CAMERA_FOLDER_PATH = Environment
			.getExternalStorageDirectory() + "/zmv" + VIDEO_FOLDER;
	public final static String TEMP_FOLDER_PATH = VIDEO_FOLDER + CAMERA_FOLDER
			+ TEMP_FOLDER;
	...
	public static File getDatabaseFolder2() {
		File root = getApplicationFolder();
		File file = null;
		if (root != null) {
			try {
				...
				file = new File(folder, SQLITE_DATABASEFILE2);
				if (!file.exists()) {
					file.createNewFile();
				}
			} catch (Exception e) {
	...
	private static String genrateFilePath(String uniqueId, boolean isFinalPath,
			File tempFolderPath) {
	...
	public static String createTempPath(File tempFolderPath) {
		long dateTaken = System.currentTimeMillis();
		String filePath = genrateFilePath(String.valueOf(dateTaken), false,
				tempFolderPath);
		return filePath;
	}
	...
	public static File getTempFolderPath() {
		File root = getApplicationFolder();
		File tempFolder = new File(root, TEMP_FOLDER_PATH + "_"
